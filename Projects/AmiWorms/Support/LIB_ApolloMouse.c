...
void ApolloMouse( ApolloMouseState* MouseState )
{
    ...
    if ( ( MouseButtonLeft_Value & 0x40 ) == 0 )
        MouseState->Button_Left = true;
    MouseButtonRight_Value = *( (volatile uint16_t*)APOLLO_MOUSE_BUTTON2 );
    if ( ( MouseButtonRight_Value & 0x400 ) == 0 )
        MouseState->Button_Right = true;
    MouseButtonMiddle_Value = *( (volatile uint16_t*)APOLLO_MOUSE_BUTTON3 );
    if ( ( MouseButtonMiddle_Value & 0x400 ) == 0 )
        MouseState->Button_Middle = true;
    ...
    if ( MouseState->MouseX_Value_Delta < -128 )
        MouseState->MouseX_Value_Delta += 256;
    if ( MouseState->MouseX_Value_Delta > 128 )
        MouseState->MouseX_Value_Delta -= 256;
    if ( MouseState->MouseY_Value_Delta < -128 )
        MouseState->MouseY_Value_Delta += 256;
    if ( MouseState->MouseY_Value_Delta > 128 )
        MouseState->MouseY_Value_Delta -= 256;
    ...
    if ( MouseState->Button_Left_Count > 0 )
        MouseState->Button_Left_Count--;
    if ( MouseState->Button_Right_Count > 0 )
        MouseState->Button_Right_Count--;
    if ( MouseState->Button_Middle_Count > 0 )
        MouseState->Button_Middle_Count--;
